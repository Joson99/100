...
function collapseUserDropdown() {
    ...
    userDropdown.classList.remove('user-dropdown-expanded');
    userDropdown.style.height = '25px';
    setTimeout(() => {
        userDropdown.classList.remove('visible'); // Remove visible class after animation
...
function toggleUserDropdown() {
    const userDropdown = document.querySelector('.user-dropdown');
    userDropdown.classList.toggle('user-dropdown-expanded');
    
    if (userDropdown.classList.contains('user-dropdown-expanded')) {
        const height = userDropdown.scrollHeight;
        userDropdown.style.height = height + 'px';
        setTimeout(() => {
            userDropdown.classList.add('visible'); // Add visible class after animation
        }, 300); // Adjust the timeout to match the transition duration
    } else {
...
loggedInUser.addEventListener('click', toggleUserDropdown);

// Add a click event listener to the document to collapse the user-dropdown when clicking outside
document.addEventListener('click', function (event) {
    const userDropdown = document.querySelector('.user-dropdown');
    if (event.target !== loggedInUser && !userDropdown.contains(event.target)) {
        collapseUserDropdown();
    }
});
